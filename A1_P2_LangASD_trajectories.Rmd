...
###Test verbalIQ
#Boxplot
ggplot(d, aes(Diagnosis, verbalIQ)) + 
  geom_boxplot ()

#Numberical test
summary(lm(verbalIQ ~ Diagnosis, d))


###Test NonverbalIQ
#Boxplot
ggplot(d, aes(Diagnosis, nonVerbalIQ)) + 
  geom_boxplot ()

#Numerical test
summary(lm(nonVerbalIQ ~ Diagnosis, d))
...
#Create model
ModelCHI = lmer(CHI_MLU ~ VISIT + Diagnosis + (1+VISIT|SUBJ), d)
#Get stats
summary(ModelCHI)
...
```{r}
#Create model
ModelMOT = lmer(MOT_MLU ~ VISIT + Diagnosis + (1+VISIT|SUBJ), d)
#Get stats
summary(ModelMOT)
